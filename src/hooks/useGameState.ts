...
const initialState: GameState = {
  supplies: [
    { id: "1", name: "Book", type: "equipment", quantity: 10 },
...
export function useGameState() {
  ...
  const [messages, setMessages] = useState<GameMessage[]>([
    {
      ...
      content: "Hello, welcome to Meet Greeter. Please enter a topic to start.",
      timestamp: Date.now(),
      type: "system",
    },
  ]);
  ...
  const addMessage = useCallback(
    (content: string, type: GameMessage["type"] = "info", image?: string) => {
      setMessages((prev) => [
        ...prev,
        {
          id: generateMessageId(),
    ...
    if (gameState.isShiftActive) {
      timerRef.current = setInterval(() => {
        setGameState((prev) => {
          ...
          const optionTimer = prev.processingOption
            ? prev.optionTimer
              ? prev.optionTimer - 1
              : undefined
            : undefined;
  ...
  useEffect(() => {
    if (gameState?.isShiftActive && gameState.lastActionTime !== undefined) {
      const checkInactivity = () => {
        if (!gameState.isShiftActive) return;
        const now = Date.now();
        const timeSinceLastAction =
          (now - (gameState?.lastActionTime || 0)) / 1000;

        if (timeSinceLastAction >= INACTIVITY_DELAY) {
          const randomQuote =
            HAWKEYE_QUOTES[Math.floor(Math.random() * HAWKEYE_QUOTES.length)];
  ...
  const startShift = useCallback(() => {
    ...
    if (!nextCase) {
      addMessage(
        "Congratulations! You have completed all available cases. More coming soon!",
        "success"
      );
      return;
    }
    ...
    addMessage(
      "🔔 New shift starting! Incoming patients...",
      "system",
      "https://i.pinimg.com/originals/3b/0c/ee/3b0cee79173dcff1f8d3308fe9e259cb.jpg"
    );
  ...
  const handleTip = useCallback(() => {
    ...
    const correctOption = gameState.currentCase.options.find(
      (opt) => opt.isCorrect
    );

    setTimeout(() => {
      addMessage(
        `Colonel Potter whispers: "${gameState.currentCase?.tip}"`,
        "info"
      );
      addMessage(
        `Hint: Consider option ${correctOption?.id.toUpperCase()}...`,
        "info"
      );
  ...
  const handleOption = useCallback(
    (optionId: string) => {
      ...
      const option = gameState.currentCase.options.find(
        (opt) => opt.id === optionId
      );
      if (!option) return;
      ...
      setTimeout(() => {
        if (option.isCorrect) {
          ...
          addMessage(
            "Patient saved! Lives saved: " + (gameState.savedLives + 1),
            "success"
          );
          ...
          addMessage(
            "Try again! The patient needs the right treatment.",
            "system"
          );
          setGameState((prev) => ({
            ...prev,
    ...
    ({ command, args }: Command) => {
      ...
      switch (command.toLowerCase()) {
        ...
        case "manual":
          ...
          addMessage(
            "\n**MASH 4077 Field Manual**\n\nThis manual is currently being written by Radar. Check back soon!",
            "info"
          );
          break;
        case "tip":
